...
class CircuitBreakerTest extends FlatSpec with Matchers with ScalaFutures {
  ...
  def assertArithExceptionAsync(hint: String, timeoutLength: Int = 100)(implicit cb: CircuitBreaker): Assertion =
  ...
  def assertCircuitException(hint: String, timeoutLength: Int = 100)(implicit cb: CircuitBreaker): Assertion =
    whenReady(Future(protectedOperation(1, 0)).failed, timeout(Span(timeoutLength, Millis))) { e =>
      withClue(s"${cb.name} : $hint") {
  ...
  def assertCircuitExceptionAsync(hint: String, timeoutLength: Int = 100)(implicit cb: CircuitBreaker): Assertion =
    whenReady(protectedAsyncOperation(1, 0).failed, timeout(Span(timeoutLength, Millis))) { e =>
      withClue(s"${cb.name} : $hint") {
        e shouldBe a[CircuitBreakerBrokenException]
      }
  ...
  def protectedOperation(x: Int, y: Int)(implicit cb: CircuitBreaker): Int = cb() {
  ...
  private def simpleBuilders(failLimit: Int = 2,
    ...
    List(
      ...
      simpleBuilder("invocation listeners both block", failLimit, retryDelay)
        .withInvocationListeners(List(new CircuitBreakerInvocationListener {
          ...
          override def onInvocationInFlowState(name: String): Unit = Thread.sleep(defaultThreadSleep)

          override def onInvocationInBrokenState(name: String): Unit = Thread.sleep(defaultThreadSleep)
        })),
      simpleBuilder(
        "invocation flow throws, invocation broken blocks",
        failLimit,
        retryDelay
      ).withInvocationListeners(List(new CircuitBreakerInvocationListener {

        override def onInvocationInFlowState(name: String): Unit = throw new Exception("boom")

        override def onInvocationInBrokenState(name: String): Unit = Thread.sleep(defaultThreadSleep)
      })),
      simpleBuilder(
        "invocation flow blocks, invocation broken blocks",
        failLimit,
        retryDelay
      ).withInvocationListeners(List(new CircuitBreakerInvocationListener {
        ...
        override def onInvocationInBrokenState(name: String): Unit = throw new Exception("boom")
      })),
      ...
      simpleBuilder("state change listeners all block", failLimit, retryDelay)
        .withStateChangeListeners(List(new CircuitBreakerStateChangeListener {
          ...
          override def onInit(name: String): Unit = Thread.sleep(defaultThreadSleep)

          override def onTrip(name: String): Unit = Thread.sleep(defaultThreadSleep)

          override def onReset(name: String): Unit = Thread.sleep(defaultThreadSleep)
        })),
      ...
      simpleBuilder(
        "state change onTrip throws, onInit, onReset block", failLimit, retryDelay)
        .withStateChangeListeners(List(new CircuitBreakerStateChangeListener {
  ...
  private def exponentialBuilders(failLimit: Int = 2,
                                  retryDelay: FiniteDuration = defaultRetryDelay,
                                  retryCap: Int = 10
                                 ): List[CircuitBreakerBuilder] = {
    List(
      exponentialBuilder("simple", failLimit, retryDelay, retryCap)
    )
  }

  private def exponentialBuilder(name: String, failLimit: Int, retryDelay: FiniteDuration,
                                 exponentialRetryCap: Int) =

    CircuitBreakerBuilder(name = name,
      failLimit = failLimit,
      retryDelay = retryDelay,
  ...
  it should "wait exponentially longer before retrying if is an exponential backoff CB" in {
    ...
    exponentialBuilders(retryDelay = Duration(baseRetryTime, TimeUnit.MILLISECONDS))
      .map(_.build())
      .foreach { x =>
        implicit val cb: CircuitBreaker = x
  ...
  it should "wait exponentially longer before retrying if is an exponential backoff CB but stop " +
    ...
    val retryCap = 2
    exponentialBuilders(retryDelay = Duration(baseRetryTime,
      TimeUnit.MILLISECONDS), retryCap = retryCap)
      .map(_.build())
      .foreach { x =>
        implicit val cb: CircuitBreaker = x
  ...
  "simple async circuit breaker" should "record failures, trip, then reset after delay time has elapsed" in {
    simpleBuilders().map(_.build()).foreach { x =>
      ...
      // now we get a circuit breaker exception because the circuit breaker is open
      assertCircuitException("CB broken failure", 10)
  ...
  it should "be 'waiting' if it is in a tripped state, but the reset time delay has been reached" in {
    simpleBuilders().map(_.build()).foreach { x =>
      ...
      (1 to 2).foreach { i => assertArithExceptionAsync(s"failure #$i") }
  ...
  "circuit breaker with fallback value" should "return the fallback value when the circuit breaker is tripped" in {
    simpleBuilders(failLimit = 5).map(_.build()).foreach { x =>
      ...
      def fallbackOperation(x: Int, y: Int) = cb(fallback = Some(Success(theDefaultValue))) {
        SimpleOperation.operation(x, y)
      }

      (1 to 5).foreach { i => assertArithException(s"failure #$i") }
  ...
  it should "throw a custom exception when configured as such and when the circuit breaker is tripped" in {
    simpleBuilders(failLimit = 5).map(_.build()).foreach { x =>
      ...
      // a bit of an odd behaviour for the fallback in this case, but tests the basic idea
      def customExceptionOperation(x: Int, y: Int) = cb(fallback = Some(customFailure)) {
        SimpleOperation.operation(x, y)
      }
  ...
  "async circuit breaker with fallback value" should "return the fallback value when the circuit breaker is tripped" in {
    simpleBuilders(failLimit = 5).map(_.build()).foreach { x =>
      ...
      // a bit of an odd behaviour for the fallback in this case, but tests the basic idea
      def fallbackOperation(x: Int, y: Int) = cb.async(fallback = Some(Success(theDefaultValue))) {
        SimpleOperation.asyncOperation(x, y)
      }
  ...
  it should "throw a custom exception when configured as such and when the circuit breaker is tripped" in {
    simpleBuilders(failLimit = 5).map(_.build()).foreach { x =>
      ...
      // a bit of an odd behaviour for the fallback in this case, but tests the basic idea
      def fallbackOperationWithCustomException(x: Int, y: Int) = cb.async(fallback = Some(customFailure)) {
        SimpleOperation.asyncOperation(x, y)
      }
  ...
  "circuit breaker configured to ignore certain exceptions" should "not be tripped when these exceptions occur" in {
    simpleBuilders()
      ...
      .foreach { x =>
        ...
        (1 to 10).foreach { i => assertArithException(s"failure #$i") }
      }
  }
  ...
  it should "not ignore exceptions that have not be configured to be ignored" in {
    simpleBuilders()
      ...
      .foreach { x =>
        ...
        // the circuit is never tripped and just lets these exceptions through
        (1 to 10).foreach { i => assertArithException(s"(non) failure #$i") }
  ...
  "async circuit breaker configured to ignore certain exceptions" should "not be tripped when these exceptions occur" in {
    simpleBuilders()
      ...
      .foreach { x =>
        ...
        // the circuit is never tripped and just lets these exceptions through
        (1 to 10).foreach { i => assertArithExceptionAsync(s"failure #$i") }
      }
  }
  ...
  "circuit breaker configured with invocation listeners" should "notify listeners on invocations" in {
    ...
    (1 to 10).foreach { _ => protectedOperation(1, 1) }
  ...
  "circuit breaker exception checking" should "let ControlThrowable exceptions through without affecting the state of the circuit breaker" in {
    ...
    (1 to 5).foreach { _ =>
      try {
        ...
        case _: CircuitBreakerBrokenException => fail("the circuit breaker should not have been tripped")
