...
class CircuitBreakerTest extends FlatSpec with Matchers with ScalaFutures {
  ...
  // for whenReady calls
  private implicit val defaultPatience = PatienceConfig(timeout = Span(2, Seconds), interval = Span(10, Millis))
  ...
  private def simpleBuilders(failLimit: Int = 2, retryDelay: FiniteDuration = defaultRetryDelay): List[CircuitBreakerBuilder] = List(
  ...
  "simple circuit breaker" should "record failures, trip, then reset after delay time has elapsed" in {
    ...
    simpleBuilders().map(_.build()).foreach { cb =>
      ...
      whenReady(Future(protectedOperation(2, 1), timeout(Span(100, Millis)))) {
        case (result, timeout) => withClue(s"${cb.name} : after retry delay") {
  ...
  it should "remain in tripped state on repeated errors" in {
    simpleBuilders(retryDelay = Duration(2, TimeUnit.SECONDS)).map(_.build()).foreach { cb =>
      ...
      def syncProtectedOperation(x: Int, y: Int): Int = Await.result(protectedOperation(x, y), Duration(1, TimeUnit.SECONDS))
  ...
  "circuit breaker with custom failure definition" should "be tripped when custom failures are detected" in {
    simpleBuilders().map( _.withResultFailureCases {
      case i: Int if i == 2 => true // whenever the returned value is 2, the circuit breaker should mark as a failure
    }.build()).foreach { cb =>
      ...
      whenReady(Future(protectedOperation(2, 1), timeout(Span(100, Millis)))) {
        case (result, timeout) => withClue(s"${cb.name} : still returns the value but records it as a failure") {
          result shouldEqual 2
        }
      }

      whenReady(Future(protectedOperation(16, 8), timeout(Span(100, Millis)))) {
        case (result, timeout) => withClue(s"${cb.name} : still returns the value but records it as a failure, trip CB") {
          result shouldEqual 2
        }
      }
  ...
  it should "ignore exceptions thrown within the check" in {
    simpleBuilders().map( _.withResultFailureCases {
      ...
      whenReady(Future(protectedOperation(), timeout(Span(100, Millis)))) {
        case (result, timeout) => withClue(s"${cb.name} : still return result when failure function throws") {
  ...
  "circuit breaker configured to ignore certain exceptions" should "not be tripped when these exceptions occur" in  {
    simpleBuilders().map( _.withNonFailureExceptionCases {
  ...
  it should "ignore exceptions thrown within the check" in {
    simpleBuilders().map( _.withNonFailureExceptionCases {
      ...
      // this should not blow up with an exception
      whenReady(Future(protectedOperation(), timeout(Span(100, Millis)))) {
        case (result, timeout) => withClue(s"${cb.name} : still return result when non-failure function throws") {
          result shouldEqual 1
        }
      }
    }
  ...
  "circuit breaker configured with invocation listeners" should "notify listeners on invocations" in {
    ...
    whenReady(promiseCheckpoint1.future, timeout(Span(50, Millis))) { case (invoked, broken) =>
    ...
    whenReady(promiseCheckpoint2.future, timeout(Span(50, Millis))) { case (invoked, broken) =>
    ...
    whenReady(promiseCheckpoint3.future, timeout(Span(50, Millis))) { case (invoked, broken) =>
    ...
    whenReady(promiseCheckpoint4.future, timeout(Span(50, Millis))) { case (invoked, broken) =>
  ...
  "circuit breaker exception checking" should "let ControlThrowable exceptions through without affecting the state of the circuit breaker" in {
    val circuitBreaker = CircuitBreakerBuilder("controlThrowable", 2, defaultRetryDelay)
      .build()
